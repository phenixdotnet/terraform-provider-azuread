...
func resourceApplicationPasswordCreate(d *schema.ResourceData, meta interface{}) error {
	...
	var existingCreds graphrbac.PasswordCredentialListResult
	if armClient.userFeatures.MSGraphAPI.IsEnabledForExistingResources {
		log.Printf("Cred Value: %s", *cred.Value)
		if *cred.Value != "" {
			return fmt.Errorf("The azuread_application_password.value mustn't be specified when using MS Graph API as the password is always generated by API")
		}

		cred, err = armClient.graphClient.CreatePasswordCredential(ctx, id.ObjectId, cred)
		log.Printf("KeyID: %s / Secret: %s", *cred.KeyID, *cred.Value)
		id = graph.PasswordCredentialIdFrom(objectId, *cred.KeyID)
		d.Set("value", *cred.Value)
	} else {
		...
		_, err = client.UpdatePasswordCredentials(ctx, id.ObjectId, graphrbac.PasswordCredentialsUpdateParameters{Value: newCreds})
	}
	...
	_, err = graph.WaitForPasswordCredentialReplication(id.KeyId, func() (graphrbac.PasswordCredentialListResult, error) {
		if armClient.userFeatures.MSGraphAPI.IsEnabledForExistingResources {
			return armClient.graphClient.ListPasswordCredentials(ctx, id.ObjectId)
		}
...
func resourceApplicationPasswordDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if armClient.userFeatures.MSGraphAPI.IsEnabledForExistingResources {
		err := armClient.graphClient.DeletePasswordCredential(ctx, id.ObjectId, id.KeyId)
		if err != nil {
